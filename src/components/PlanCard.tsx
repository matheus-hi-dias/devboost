...
export function PlanCard({ title, features, price, highlighted }: PlanCardProps) {
  return (
    <div
    ...
    >
      <div>
        <h3 className={`text-xl font-bold mb-4 ${highlighted ? "text-yellow-700" : "text-primary"}`}>
          {title}
        </h3>
        <ul className={`space-y-2 mb-8 list-disc list-inside ${highlighted ? "text-gray-700" : "text-muted-foreground"}`}>
          {features.map((feature, index) => (
            <li key={index}>{feature}</li>
          ))}
        </ul>
      </div>
      <span className={`text-3xl font-black ${highlighted ? "text-yellow-800" : "text-secondary-foreground "}`}>
